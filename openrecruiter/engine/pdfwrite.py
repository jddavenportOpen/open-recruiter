...
# Substitution counting is CONTEXT-LOCAL, not a module global.
#
# It used to be a module-level dict that render() cleared on entry. That is
# correct for one-at-a-time CLI use and silently wrong the moment two documents
# render concurrently: a clean resume inherits the other document's counts and
# reports "the typesetter changed this" about characters it never contained.
# Measured before this fix -- a clean doc rendered beside a dirty one reported 4
# substitutions belonging to the other document. A queue hits that immediately,
# and provenance notes attached to the wrong application are worse than none.
#
# A ContextVar isolates per-thread AND per-asyncio-task: a fresh thread starts
# with an empty context, so it cannot see another render's counts.
_SUBS: contextvars.ContextVar[dict] = contextvars.ContextVar("pdfwrite_substitutions")
...
def substitution_scope():
    """Isolate substitution counting to this block. Yields the dict it collects into."""
    token = _SUBS.set({})
    try:
        yield _SUBS.get()
    finally:
...
def substitutions() -> dict:
    """A copy of what has been counted in the current context."""
    return dict(_counts())
...
def sanitize(text: str) -> str:
    ...
    for bad, good in TRANSLIT.items():
        if bad in text:
            c = _counts()
            c[bad] = c.get(bad, 0) + text.count(bad)
