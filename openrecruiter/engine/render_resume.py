...
def render(resume: dict, out_path: str) -> dict:
    with P.substitution_scope() as _subs:
        return _render(resume, out_path, _subs)


def _render(resume: dict, out_path: str, _subs: dict) -> dict:
    ...
    # A flat list is a natural thing for a caller to hand us (and the example
    # banks use `skills_pool`, which IS a list). It used to raise AttributeError
    # deep in the renderer, which reads as "the PDF engine is broken" rather than
    # "your skills had no categories". Categorise it as one unlabelled group.
    ...
    with open(out_path, "wb") as f:
        f.write(data)
    for ch, n in sorted(_subs.items()):
