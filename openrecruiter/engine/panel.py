...
The three calibration mechanics (documented in the README):
  ...
  2. Vote-coupling, BOUNDED: a persona voting "would interview" is lifted toward
     ADVANCE_FLOOR so residual mid-band compression can't sink a genuine yes --
     but the lift is capped at MAX_VOTE_LIFT points, because a vote is a vote and
     not a score. Unbounded `max(s, 85)` made three yes-votes an unconditional
     pass at the standard tier: a panel scoring ZERO on every dimension returned
     panel_avg 85.0 and overall_pass True. Measured, not theorised. Anything that
     reads overall_pass to decide whether to SEND would have been sending on
     three booleans.
  3. Raw floor: the unclamped composite must also stand on its own (RAW_FLOOR_*).
     The lift cap alone still lets a vote carry a weak panel; this is the guard
     that makes the five weighted dimensions load-bearing in the majority case.
  4. Tiered pass: reach employers need a majority interview vote AND panel >= 90;
     everyone else needs panel >= 70. Both tiers also need their raw floor.
"""
import json
import sys
...
ADVANCE_FLOOR = 85
# A yes-vote may lift a persona's score toward ADVANCE_FLOOR, but by at most this
# many points. 15 keeps the case the floor was built for (a judge who votes yes
# and then scores 72-78 is lifted to 85) while refusing the pathological one (a
# judge who votes yes and scores 10 is lifted to 25, not 85).
MAX_VOTE_LIFT = 15
REACH_THRESHOLD = 90
DEFAULT_THRESHOLD = 70
# The UNCLAMPED composite has to clear its own bar as well. Without this a vote
# can still carry a weak panel over the coupled threshold.
RAW_FLOOR_REACH = 80
RAW_FLOOR_DEFAULT = 68
...
def _coupled(score):
    """A yes-vote lifts a score toward ADVANCE_FLOOR, by at most MAX_VOTE_LIFT.

    Never below the score itself (a vote must not HURT), never above the floor
    (a vote is not evidence of excellence), and never more than the cap (a vote
    cannot manufacture a score out of nothing)."""
    return max(score, min(ADVANCE_FLOOR, score + MAX_VOTE_LIFT))
...
def aggregate(payload):
    ...
    effective = [_coupled(scores[p]) if votes[p] else scores[p] for p in PANEL]
    panel_avg = round(sum(effective) / len(PANEL), 1)
    interview_votes = sum(votes.values())
    reach = is_reach(company)
    threshold = REACH_THRESHOLD if reach else DEFAULT_THRESHOLD
    raw_floor = RAW_FLOOR_REACH if reach else RAW_FLOOR_DEFAULT
    if reach:
        passed = interview_votes >= 2 and panel_avg >= threshold and raw >= raw_floor
    else:
        passed = panel_avg >= threshold and raw >= raw_floor
    weakest = min(PANEL, key=lambda p: scores[p])
    return {
        "company": company, "is_reach": reach, "threshold": threshold,
        "raw_floor": raw_floor,
