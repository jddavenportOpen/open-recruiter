...
def extract(path: str) -> tuple[str, str]:
    ...
    try:
        ...
        pass
    # Context-managed on purpose: this runs once per application in a loop that
    # is meant to stay up all day, and a leaked descriptor per resume adds up.
    with open(path, "rb") as fh:
        return text_from_bytes(fh.read()), "stdlib"
